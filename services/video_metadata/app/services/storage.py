...
class S3Service:
    def __init__(self):
        # Ініціалізуємо асинхронну сесію
    ...
    async def ensure_bucket_exists(self) -> None:
        """Перевіряє, чи існує бакет, і створює його, якщо ні."""
        # type: ignore - Pylance doesn't recognize aioboto3 async context manager properly
        # type: ignore
    ...
    async def upload_video(self, file_obj: BinaryIO, object_name: str, content_type: str = "video/mp4") -> str:
        ...
        Використовує upload_fileobj для потокового завантаження.
        """
        # type: ignore - Pylance doesn't recognize aioboto3 async context manager properly
        # type: ignore
        async with self.session.client("s3", endpoint_url=self.endpoint_url) as client:
            try:
                # ExtraArgs дозволяє вказати тип файлу, щоб MinIO/S3 правильно його віддавав
